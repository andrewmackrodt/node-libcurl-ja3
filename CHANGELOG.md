...
### Fixed
- Fix default impersonate headers when setting user defined headers

## [5.2.1] - 2025-12-22

### Fixed
- Added `curly` response property `info` when returning a stream response

## [5.2.0] - 2025-12-21

### Breaking Change
- Renamed camelCase `CurlOption` property `httpbaseheader` to `httpBaseHeader`

### Added
- Added `curly` request property `curlyGetInfo` which returns an `info` object containing all output from `Curl.getInfo`
- Add support for the following curl-impersonate options:
  - `CURLOPT_HTTP2_NO_PRIORITY`
  - `CURLOPT_IMPERSONATE`
  - `CURLOPT_PROXY_CREDENTIAL_NO_REUSE`
...
### Changed
- Updated default impersonate profiles to:
  - Chrome 143
  - Edge 143
  - Firefox 144
  - Safari 18.6
...
### Added
- Specify custom impersonate config to `Curl.impersonate` method

### Fixed
- Fix specifying custom impersonate config to `curly.impersonate`
...
### Fixed
- Using Stream Response with HTTP2 connections cause a hang [#1](https://github.com/andrewmackrodt/node-libcurl-ja3/issues/1)
...
[5.2.2]: https://github.com/andrewmackrodt/node-libcurl-ja3/compare/v5.2.1...HEAD
